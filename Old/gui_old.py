# gui.py
# Main test GUI script - old version
...
# Class representing telemetry data from the PCB - state, temperature, voltage, fault code
class Telemetry:
...
class MainWindow():
    ...
    # Start timeout timer here?
        ...
        self.rows_filled = np.ndarray((IMAGE_HEIGHT, PKTS_PER_ROW), dtype=bool)
        ...
        while (True): 
            ...
            if (data_hex != None):
                ...
                if (header.casefold() == SIG_IMAGE_DATA.casefold()):
                    if (len(data_hex) > 1000): # Length of an image data packet - normally 6144+8 (8 + 1536 + 2)
                        ...
                        for i in range(0, len(imageData), 3):
                            self.frame[rowIndex, (i // 3) + (colIndex * IMAGE_WIDTH // 4)] = int(imageData[i:i+3], 16)
                        
                        self.rows_filled[rowIndex, colIndex] = True 
    ...
    def read_image(self):
        if (self.transmission_udp and self.sock != None):
            ...
            self.log_to_file("Sent write to flash, UDP")
        else:
            self.log_to_file("Failed to send write to flash - no connection established")
